...
st.set_page_config(page_title="DataMap AI", page_icon="frilogo.png",
...
if 'session_button' not in st.session_state:
    st.session_state.session_button = [
...
with main_page_sidebar:
    ...
    st.session_state.tab_selection = option_menu(
        ...
        options=['Home', 'Add Key', 'Upload files', 'Begin Chat',
                 '---', '---', '---', '---', 'Contact us'],
        icons=['house-door-fill', 'key-fill', 'database-fill-add',
               'chat-fill', '', '', '', '', 'send-plus-fill', 'box-arrow-in-right'],
...
def test_embed():
    ...
    if 'index' not in st.session_state:
        st.session_state.index = FAISS.from_documents([Document(
            page_content='hi', metadata={'source': 'one).pdf', 'page': 1})], embeddings)
    # Save in a Vector DB
    # index =[]
    ...
    try:
        if len(st.session_state.api) > 1:
            if not st.session_state.pages:
                ...
                st.header(
                    "Supported: PDF, Docx, PNG, CSV, Youtube & Website URLs")
                with st.expander("Upload Files"):
                    urlinput = st.text_input(
                        "Youtube Links", placeholder="Enter Youtube links separated by spaces")
                    urlinput1 = st.text_input(
                        "Website Link", placeholder="jsmastery.pro", help='URL format: jsmastery.pro')

                    uploaded_file = st.file_uploader("**Supported Formats: PDF, Docx, PNG (Extracts text from image)**",
                                                     accept_multiple_files=True, help='Uploaded files will not be stored', type=['png', 'pdf', 'csv', 'docx'])
...
if st.session_state.tab_selection == "Begin Chat":
    st.session_state.option = st.selectbox(
        '',
        ('General Query: Flowchart', 'Uploaded data: Flowchart', 'Uploaded data: Quick Query', 'Uploaded data: Conversational Chat', 'General Query: Mindmap'), label_visibility="collapsed", )
        ...
        try:
            if st.session_state.agree:
                ...
                prompt1 = ZeroShotAgent.create_prompt(
                    ...
                    suffix=suffix1,
                    input_variables=[
                        "input1", "chat_history1", "agent_scratchpad"],
                )
                ...
                agent1 = ZeroShotAgent(
                    llm_chain=llm_chain1, tools=tools5, verbose=True)
                            ...
                            try:
                                ...
                                if st.session_state.option.strip() == 'Uploaded data: Flowchart':
                                    try:
                                        ...
                                        llm_response1 = '''digraph{''' + \
                                            llm_response+'''} diagramcreator'''
                                    except Exception as e:
                                        # st.write(e)
                    ...
                    with chat_placeholder:
                        for chat in st.session_state.history:
                            ...
                            if chat.origin == 'ai' and st.session_state.isdiagram == True:
                                content = st.graphviz_chart(
                                    message_before_last_word)
                                content = ""
                            else:
...
resp = client.
                    ...
                    class MindMap:
                        ...
                        def ask_for_initial_graph(self, query: str) -> None:
                            ...
                            output, self.conversation = ask_chatgpt(
                                conversation)
                        ...
                        def parse_and_include_edges(self, output: str, replace: bool = True) -> None:
                            ...
                            matches = re.findall(
                                pattern1, output) + re.findall(pattern2, output)
                            ...
                            self.nodes = list(
                                set([n for e in self.edges for n in e]))
                            self.save()
                        ...
                        def _delete_node(self, node) -> None:
                            ...
                            """
                            self.edges = [
                                e for e in self.edges if node not in frozenset(e)]
                            self.nodes = list(
                                set([n for e in self.edges for n in e]))
                        ...
                        def visualize(self, graph_type: Literal["agraph", "networkx", "graphviz"]) -> None:
                            ...
                            if graph_type == "agraph":
                                ...
                                vis_edges = [Edge(source=a, target=b)
                                ...
                                if clicked_node is not None:
                                    self._add_expand_delete_buttons(
                                        clicked_node)
                                return
                            if graph_type == "networkx":
                                ...
                                for a, b in self.edges:
                                    graph.add_edge(a, b)
                                colors = [FOCUS_COLOR if node ==
                                          selected else COLOR for node in graph]
                                ...
                                for n in self.nodes:
                                    graph.node(
                                        n, style="filled", fillcolor=FOCUS_COLOR if n == selected else COLOR)
                                # st.graphviz_chart(graph, use_container_width=True)
                                b64 = base64.b64encode(graph.pipe(
                                    format='svg')).decode("utf-8")
...
if st.session_state.tab_selection == "Add Key":
    ...
    if len(st.session_state.api) > 1:
        ...
        st.session_state.api = st.text_input(
            "API Key ", placeholder="Enter your API Key", type='password', help='API Keys will not be stored')
        if st.session_state.api:
            st.rerun()
